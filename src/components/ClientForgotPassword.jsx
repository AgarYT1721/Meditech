...
const ClientForgotPassword = ({ onBackToLogin }) => {
  ...
  return (
    <div className="mobile-auth-wrapper" style={{
      ...
      <motion.div layoutId="auth-card" className="mobile-auth-container" style={{
        width: '100%', maxWidth: '400px', background: 'rgba(255, 255, 255, 0.85)', backdropFilter: 'blur(20px)', borderRadius: '24px', padding: '40px', boxShadow: '0 20px 40px rgba(0,0,0,0.05), inset 0 0 0 1px rgba(255,255,255,0.6)', position: 'relative', zIndex: 10, margin: '20px'
      }}>
        <AnimatePresence mode="wait">
          {!isSuccess ? (
            <motion.div key="form" variants={containerVariants} initial="hidden" animate="visible" exit="exit">
              <motion.div variants={itemVariants}>
                <button onClick={onBackToLogin} style={{ background: 'transparent', border: 'none', color: '#888', display: 'flex', alignItems: 'center', gap: '5px', cursor: 'pointer', marginBottom: '30px', fontSize: '0.9rem', padding: 0 }}>
              ...
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                ...
                <motion.div variants={itemVariants}>
                  <button type="submit" disabled={isLoading} style={{
                    ...
                    {isLoading ? <Loader2 size={20} style={{ animation: 'spin 1s linear infinite' }} /> : 'Send Reset Link'}
                  </button>
                </motion.div>
              </form>
            </motion.div>
          ) : (
            <motion.div key="success" variants={containerVariants} initial="hidden" animate="visible" exit="exit" style={{ textAlign: 'center', padding: '40px 0' }}>
              ...
              <motion.h1 variants={itemVariants} style={{ margin: 0, fontSize: '1.8rem', color: '#181818', fontWeight: 800 }}>Email Sent!</motion.h1>
              <motion.p variants={itemVariants} style={{ margin: '15px 0 30px 0', color: '#666', fontSize: '1rem', lineHeight: 1.5 }}>
                We've sent a secure password reset link to <strong style={{ color: '#181818' }}>{email}</strong>. Please check your inbox and follow the instructions.
              </motion.p>
              <motion.div variants={itemVariants}>
                <button onClick={onBackToLogin} style={{
                  width: '100%', padding: '16px', background: '#f8fafc', color: '#181818', border: '1px solid rgba(0,0,0,0.1)', borderRadius: '12px', fontSize: '1.05rem', fontWeight: 'bold', cursor: 'pointer'
                }}>
                  Return to Sign In
                </button>
              </motion.div>
